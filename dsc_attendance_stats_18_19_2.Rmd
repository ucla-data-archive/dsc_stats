...
event <- event[moveme(names(event), "Attendee.Dept.program before Barcode")]
...
event <- event[c(1:10, 26, 11:18, 32, 33, 30, 31, 34, 35, 19, 20, 27, 28, 21, 22, 24, 25, 29, 23, 36, 37, 38, 39)]

names(event) # Should reflect variable order in events
...
event2 <- select(event2,-c(Setup, Teardown, Tags, X10, Notes, Organizer, Phone, Barcode, `Note:`, First, Last, Email))  #eliminate useless variables for analysis
...
#Personal Data
event2 <- event2 %>% filter(!is.na(Event.Status)) #eliminate 28 people who put down personal information
event2 <- event2 %>% mutate_at(vars("Attendee.Occupation", "Attendee.Dept.program", "Email.opt.in?"), ~replace_na(.,"No"))

#Duration
#Eliminate day-long/multi-day events
event2<-event2[!(event2$Title=="GIS Day"|event2$Title=="Love Data Week 2019"|event2$Title=="Software Carpentry Workshop - Python - DAY 1"|event2$Title=="Software Carpentry Workshop - Python - DAY 2"),]
...
#Only NA's left given to Python Fundamentals event
...
event3 <- select(event2, -c("Created"))

library(xlsx)
write.xlsx(event3, file="clean_data.xlsx", 
           sheetName="DSC_Stats_18_19")
```
